...
class FeedbackAgent:
    ...
    async def evaluate(self, part_type: PartType, question: ExaminerQuestion, user_answer: str) -> FeedbackResult:
        raw = await self.llm.chat(build_feedback_prompt(part_type, question, user_answer), temperature=0.2)
        try:
