...
> 当前没有真实发音分析，Pronunciation 返回 N/A；开发环境仍可使用返回固定 transcript 的 Mock ASR。
...
flowchart LR
  ...
  B --> D[ASR Provider]
  D --> E[Transcript]
  E --> F[DeepSeek Scoring]
  F --> G[(SQLite)]
  B --> H[(Local Audio Storage)]
```
...
4. ASR Provider 返回 transcript，DeepSeek 对文本进行结构化评分。
5. 后端保存录音、transcript、反馈和分数；发音维度保持 N/A。
6. Full Mock 每题先转写评分，8 题完成后再生成一次全局报告。
...
当前真实评分仍基于 transcript。Gemini ASR 会先将 WebM/MP4 等浏览器录音转成 16 kHz 单声道 WAV，再调用 Gemini Audio。OpenAI ASR、真实发音分析、用户账号、云对象存储和部署配置仍属于后续范围；原文字评分 API 保留作为 fallback。
