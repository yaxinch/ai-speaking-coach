...
7. 新版 Full Mock 从审核题库组成 6/1/4 共 11 题；每次 Next 都先完成当前题的转写、发音评估和文本评分，Finish Test 只执行确定性本地聚合与归档，不再发起整场 LLM 请求。Finish Test 进度按已有录音题数显示，因此最后一题录音完成后为 `11/11`；评分状态仍独立用于防止重复调用 Provider。旧 4/1/3 数据结构继续兼容。
