...
export function SpokenQuestionCard({
  ...
  const label = voiceState?.isLoading ? "Generating examiner voice..." : voiceState?.isPlaying ? "Playing examiner voice..." : "Play Examiner Question";
  return (
    <Panel>
      <Stack spacing={2}>
        <Box>
        ...
        <Button variant="contained" startIcon={<VolumeUpOutlinedIcon />} onClick={onPlay} disabled={disabled || voiceState?.isLoading || voiceState?.isPlaying} sx={{ alignSelf: "flex-start" }}>
          {label}
        </Button>
